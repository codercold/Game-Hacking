...
int lde( void *addr )
{
	...
	_CodeInfo code_info = { 0, 0, (uint8_t*)addr, 16, Decode32Bits };
...
//static int hook_create_trampoline( unsigned char *addr, int len, unsigned char *tramp )
//{
//	Log( "Entered hook_create_trampoline\n" );
//	Log( "param_01: %08X", addr );
//	Log( "param_02: %d", len );
//	Log( "param_03: %08X", tramp );
//    const unsigned char *base = tramp;
//	Log( "base/tramp address: %08X", base );
//
...
//
//    volatile unsigned char pre_backup[] = 
//	{
//        // push eax
//        0x50,
...
//    };
//
//	Log("pre_backup Address: %08X", pre_backup);
//	for( int i = 0; i < ( sizeof(pre_backup) / sizeof(pre_backup[0]) ); i++ )
//	{
//		Log( "pre_backup: %02X", pre_backup[i] );
//	}
//	Log("\n");
//	
//    // The function returns here after executing, backup the Last Error Code
//    volatile unsigned char post_backup[] = 
//	{
//        // push eax
//        0x50,
//        // mov eax, fs:[TLS_HOOK_INFO]
...
//    };
//
//	Log("post_backup Address: %08X", post_backup);
//	for( int i = 0; i < ( sizeof(post_backup) / sizeof(post_backup[0]) ); i++ )
//	{
//		Log( "post_backup element data: %02X", post_backup[i] );
//	}
//	Log("\n");
//
//	Log( "&ensure_valid_hook_info - tramp: %08X", ( (unsigned char *)&ensure_valid_hook_info - tramp - 12 - 5 ) );
//    *(unsigned int *)( pre_backup + 13 ) = (unsigned char *) &ensure_valid_hook_info - tramp - 12 - 5;
//	Log( "pre_backup + 13: %08X: ", *(unsigned int *)( pre_backup + 13 ) );
//	Log( "pre_backup + 13: %08X: ",  (unsigned int *)( pre_backup + 13 ) );
//	Log("\n");
//
//	//memcpy over size of pre_backup, first array.
//    memcpy( tramp, (const void*)pre_backup, sizeof( pre_backup ) );
//	Log( "tramp: %08X", tramp );
//
//    tramp += sizeof( pre_backup );
//	Log( "tramp + sizeof( pre_backup ): %08X", tramp );
//
//    unsigned char **pre_backup_addr = (unsigned char **)( tramp - 5 );
//	
//	Log( "**pre_backup_addr: %08X", pre_backup_addr );
//
//    // our trampoline should contain at least enough bytes to fit the given length	
//    while ( len > 0 ) 
//	{
//		Log("len used in while loop: %d", len );
//
//        // obtain the length of this instruction
//        int length = lde( addr );
//		Log( "Obtain Length Of Instruction: %d", length );
//
//        // error?
//        if( length == 0 ) 
//		{
...
//        // how many bytes left?
//        len -= length;
//		Log("Subtract length from len equals: %d", len );
//
...
//        if( *addr == 0xe9 || *addr == 0xe8 || ( *addr == 0x0f && addr[ 1 ] >= 0x80 && addr[ 1 ] < 0x90 ) ) 
//		{
//			Log("conditional jump or call found with 32 bit offset");
//            // copy the jmp or call instruction ( conditional jumps are two bytes, the rest is one byte )
//            *tramp++ += *addr++;
...
//            unsigned long jmp_addr = *(unsigned long *) addr + 4 + (unsigned long) addr;
//            addr += 4;
//
...
//		{
//			Log( "conditional jump with 8bit offset found" );
...
//
//            // 8bit relative offset, we have to sign-extend it (by casting it as signed char) 
//			// in order to calculate the correct address
//            unsigned long jmp_addr = (unsigned long) addr + 2 + *(signed char *)( addr + 1 );
//			
...
//            while ( length-- != 0 ) 
//			{
//				Log("Copy instruction directly into trampoline");
//                *tramp++ = *addr++;
//            }
//        }
//    }
...
//	Log( "Before appending jmp to tramp: %08X", tramp );
//    *tramp++ = 0xe9;
//    *(unsigned int *)tramp = (unsigned int)addr - (unsigned int)tramp - 4;
//	Log( "After appending jmp to tramp: %08X", *(unsigned int *)tramp );
//    tramp += 4;
//	Log( "tramp +4: %08X", tramp );
//
//    // return address is the next instruction after the jmp
//    *pre_backup_addr = tramp;
//	Log( "pre_backup_addr: %d", *pre_backup_addr );
//
//    memcpy( tramp, (const void*)post_backup, sizeof(post_backup) );
//    tramp += sizeof(post_backup);
//
//	Log( "Length of tramp: %d", tramp - base );
//
...
{
	Log( "is_special_hook: %d", is_special_hook );
  ...
  /*01*/   0x50,
           // mov eax, fs:[TLS_HOOK_INFO]
  /*02*/   0x64, 0xa1, TLS_HOOK_INFO, 0x00, 0x00, 0x00,
           // test eax, eax
  /*08*/   0x85, 0xc0,
           // jnz $+0d
  /*10*/   0x75, 0x0d,
  ...
  /*12*/       0x60,
               // call ensure_valid_hook_info
  /*13*/       0xe8, 0x00, 0x00, 0x00, 0x00,                        /* +14 Fill In below */
               // popad
  /*18*/       0x61,
               // mov eax, fs:[TLS_HOOK_INFO]
  /*19*/       0x64, 0xa1, TLS_HOOK_INFO, 0x00, 0x00, 0x00,
		   
           // inc dword [eax+hook_info_t.depth_count]
  /*25*/   0xff, 0x40, offsetof(hook_info_t, depth_count),           // depth count 1
           // mov ebx, [esp+8]
  /*28*/   0x8b, 0x5c, 0xe4, 0x08,                                   // Normal Present Return address.... 
           // xchg esp, [eax+hook_info_t.retaddr_esp]
  /*32*/   0x87, 0x60, offsetof(hook_info_t, retaddr_esp),
           // push ebx
  /*35*/   0x53,
           // xchg esp, [eax+hook_info_t.retaddr_esp]
  /*36*/   0x87, 0x60, offsetof(hook_info_t, retaddr_esp),           // offset of hook_info_t struct's member retaddr_esp nice function.. 
           // mov dword [esp+8], new_return_address
  /*39*/   0xc7, 0x44, 0xe4, 0x08, 0x00, 0x00, 0x00, 0x00,          /* +43 Fill In below */   
           // mov ebx, 1
  /*47*/   0xbb, 0x01, 0x00, 0x00, 0x00,                             // special hook support I think bb is wrong here, should be b8 ?
           // cmp ebx, is_special_hook
  /*52*/   0x83, 0xfb, is_special_hook,
           // jnz $+7
  /*55*/   0x75, 0x07,
  ...
  /*58*/       0x5b,
               // jmp h->store_exc
  /*59*/       0xe9, 0x00, 0x00, 0x00, 0x00,                        /* +60 Fill In below */
		   
           // cmp dword [eax+hook_info_t.depth_count], 1
  /*64*/   0x83, 0x78, offsetof(hook_info_t, depth_count), 0x01,
           // jle $+7
  /*68*/   0x7e, 0x07,
  ...
  /*71*/  	   0x5b,
               // jmp h->tramp
  /*72*/       0xe9, 0x00, 0x00, 0x00, 0x00,                        /* +73 Fill In below */
  ...
  /*79*/   0xe8, 0x00, 0x00, 0x00, 0x00,                            /* +80 Fill In below */
           // test eax, eax
  /*84*/   0x85, 0xc0,
           // pop eax
  /*86*/   0x58,
  ...
  /*90*/       0x58, 0x5b,
               // jmp h->tramp
  /*92*/       0xe9, 0x00, 0x00, 0x00, 0x00,
		   
           // pop eax; pop ebx
  /*97*/   0x58, 0x5b,
           // jmp h->store_exc
  /*99*/   0xe9, 0x00, 0x00, 0x00, 0x00,                            /* +100 Fill In below */
		   
		   
           // push ebx; push eax
  /*104*/  0x53, 0x50,
           // mov eax, fs:[TLS_HOOK_INFO]
  /*106*/  0x64, 0xa1, TLS_HOOK_INFO, 0x00, 0x00, 0x00,
           // dec dword [eax+hook_info_t.depth_count]
  /*112*/  0xff, 0x48, offsetof(hook_info_t, depth_count),
           // push dword [eax+hook_info_t.last_error]
  /*115*/  0xff, 0x70, offsetof(hook_info_t, last_error),
           // pop dword fs:[0x34]
  /*118*/  0x64, 0x8f, 0x05, 0x34, 0x00, 0x00, 0x00,		   
           // xchg esp, [eax+hook_info_t.retaddr_esp]
  /*125*/  0x87, 0x60, offsetof(hook_info_t, retaddr_esp),
           // pop ebx
  /*128*/  0x5b,
           // xchg esp, [eax+hook_info_t.retaddr_esp]
  /*128*/  0x87, 0x60, offsetof(hook_info_t, retaddr_esp),
           // pop eax
  /*132*/  0x58,
           // xchg ebx, dword [esp]
  /*133*/  0x87, 0x1c, 0xe4,
  ...
  /*136*/  0xc3,
	...
	Log( "pre_tramp: %08X", pre_tramp );
	Log( "pre_tramp size: %d", sizeof(pre_tramp) );

    *(unsigned int *)( pre_tramp + 14  ) = (unsigned char *) &ensure_valid_hook_info - h->pre_tramp - 13 - 5;
	Log( "pre_tramp + 14: %08X", *(unsigned int *)( pre_tramp + 14  ) );

    *(unsigned int *)( pre_tramp + 43  ) = (unsigned int)    h->pre_tramp + 104;
	Log( "pre_tramp + 43: %08X", *(unsigned int *)( pre_tramp + 43  ) );

    *(unsigned int *)( pre_tramp + 60  ) = (unsigned char *) h->store_exc - h->pre_tramp - 59 - 5;
	Log( "pre_tramp + 60: %08X", *(unsigned int *)( pre_tramp + 60  ) );

	*(unsigned int *)( pre_tramp + 73  ) =                   h->tramp - h->pre_tramp - 72 - 5;
	Log( "pre_tramp + 73: %08X", *(unsigned int *)( pre_tramp + 73  ) );

    *(unsigned int *)( pre_tramp + 80  ) = (unsigned char *) &is_interesting_backtrace - h->pre_tramp - 79 - 5;
	Log( "pre_tramp + 80: %08X",  *(unsigned int *)( pre_tramp + 80 ) );

    *(unsigned int *)( pre_tramp + 93  ) =                   h->tramp - h->pre_tramp - 92 - 5;
	Log( "pre_tramp + 93: %08X",  *(unsigned int *)( pre_tramp + 93 ) );

    *(unsigned int *)( pre_tramp + 100 ) = (unsigned char *) h->store_exc - h->pre_tramp - 99 - 5;
	Log( "pre_tramp + 100: %08X",  *(unsigned int *)( pre_tramp + 100 ) );
...
static void hook_store_exception_info(hook_t *h)
{
    ...
    *(unsigned int *)( store_exception + offset + 1 ) = (unsigned char *)h->new_func - h->store_exc - offset - 5;
...
static int hook_api_mov_eax_push_retn(hook_t *h, unsigned char *from, unsigned char *to)
{
    // mov eax, address
    *from++ = 0xb8;
    *(unsigned char **) from = to;
...
static int hook_api_special_jmp( hook_t *h, unsigned char *from, unsigned char *to )
{
...
int hook_api( hook_t* h, int type )
{
    ...
    static struct 
	{
        ...
        int len;
         ...
         { &hook_api_jmp_direct,                 5 }, /* HOOK_JMP_DIRECT                 */
         { &hook_api_nop_jmp_direct,             6 }, /* HOOK_NOP_JMP_DIRECT             */
         { &hook_api_hotpatch_jmp_direct,        7 }, /* HOOK_HOTPATCH_JMP_DIRECT        */
         { &hook_api_push_retn,                  6 }, /* HOOK_PUSH_RETN                  */ //we can use
         { &hook_api_nop_push_retn,              7 }, /* HOOK_NOP_PUSH_RETN              */
         { &hook_api_jmp_indirect,               6 }, /* HOOK_JMP_INDIRECT               */
         { &hook_api_mov_eax_jmp_eax,            7 }, /* HOOK_MOV_EAX_JMP_EAX            */
         { &hook_api_mov_eax_push_retn,          8 }, /* HOOK_MOV_EAX_PUSH_RETN          */ //we can use 
         { &hook_api_mov_eax_indirect_jmp_eax,   7 }, /* HOOK_MOV_EAX_INDIRECT_JMP_EAX   */
         { &hook_api_mov_eax_indirect_push_retn, 7 }, /* HOOK_MOV_EAX_INDIRECT_PUSH_RETN */
#if HOOK_ENABLE_FPU
         { &hook_api_push_fpu_retn,             11 }, /* HOOK_PUSH_FPU_RETN              */
#endif
         { &hook_api_special_jmp,                7 }, /* HOOK_SPECIAL_JMP                */
    };
	...
	Log("Check hook_t members");
	Log("const wchar_t* h->library: 0x%08X", h->library );
	Log("const char* h->funcname: 0x%08X", h->funcname );
	Log("void*  h->addr: 0x%08X", h->addr );
	//Log("void*  h->new_func 0x%08X\n", h->new_func );
	
	//DWORD* addrTest = (DWORD*)h->addr;	
	//printf("DWORD* addrTest: %X\n", addrTest );
    //unsigned char *addr = (unsigned char *)(*addrTest);

	// resolve the address to hook
    unsigned char *addr = (unsigned char *)h->addr;
	Log("unsigned char addr: 0x%08X", addr );

    if( addr == NULL && h->library != NULL && h->funcname != NULL ) 
	{		
        addr = (unsigned char *) GetProcAddress( GetModuleHandleW( h->library ), h->funcname );
		Log("GetProcAddress Return Value: 0x%08X", addr );
    }
    ...
    if( type >= 0 && type < ARRAYSIZE( hook_types ) ) 
	{
        ...
        if( GetVersionEx( &os_info ) && os_info.dwMajorVersion == 6 && os_info.dwMinorVersion == 1 ) 
		{
            ...
            // two jumps.
            if( !memcmp( addr, "\xeb\x05", 2 ) && !memcmp( addr + 7, "\xff\x25", 2 ) ) 
			{
				// block is all zero ???
				Log( "\ninside layer" );
            ...
            else if( !memcmp( addr, "\x8b\xff\x55\x8b\xec\x5d\xeb\x05", 8 ) && !memcmp( addr + 13, "\xff\x25", 2 ) ) 
			{
				Log( "\ninside patch cuckoo" );
            ...
            // inlined function.
            if( !memcmp( addr, "\xeb\x02", 2 ) && !memcmp( addr - 5, "\xcc\xcc\xcc\xcc\xcc", 5 ) ) 
			{
				Log( "\ninside inlined" );
		...
		{
			Log("\nBefore create tramp addr: %08X", addr );
            if( hook_create_trampoline( addr, hook_types[type].len, h->tramp ) ) 
			{
                ...
                // insert the hook ( jump from the api to the pre-trampoline )
...
static void ensure_valid_hook_info()
{
    if( hook_info() == NULL ) 
	{
        hook_info_t *info = (hook_info_t *) calloc( 1, sizeof( hook_info_t ) + TLS_HOOK_INFO_RETADDR_SPACE ); /* 13312 Bytes ??? */
