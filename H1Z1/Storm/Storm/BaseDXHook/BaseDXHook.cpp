...
// --- DeviceSetup ---
HRESULT DeviceSetup( HWND hwnd, DWORD64* Vtab )
{
	...
	DWORD64* pVTable = (DWORD64*)pd3dDevice;

    // Put the dword address into pVTable for referencing these 118 direct3d devices functions. 
	pVTable = (DWORD64*)pVTable[0];
	//Log( "VTable Pointer: %016llX", pVTable ) ;
	//Log( "VTable[17] Present Address: %016llX", pVTable[17]  ) ;
	//Log( "VTable[42] End Scene Address: %016llX", pVTable[42]  ) ;
	//Log( "VTable[82] DrawIndexedPrimitive Address: %016llX", pVTable[82]  ) ;
	//Log( "VTable[100] SetStreamSource Address: %016llX", pVTable[100] ) ;
	...
	//Log( "Functions Assigned" );
	//Log( "End Of The DeviceSetup Function" );
...
VOID OverlayWindow( DWORD64* Vtable )
{
   ...
   // return type - If the function succeeds, the return value is a handle to the new window.
	...
	//Log ( "End Of The OverlayWindow() Function\n" ) ;
}
